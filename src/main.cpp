...
{	
		...
		DataIdx++)
	{
		...
		CommandData.Translate = vec4((float(rand()) / RAND_MAX) * 4 - 2, 
									 (float(rand()) / RAND_MAX) * 4 - 2, 
									 (float(rand()) / RAND_MAX) * 4 - 2, 0.0f);
	}
	...
	buffer IndirectDrawCommands(DirectWindow.Gfx, IndirectDrawCommandVector.data(), sizeof(indirect_draw_command) * DrawCount, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	buffer IndirectDrawIndexedCommands(DirectWindow.Gfx, IndirectDrawIndexedCommandVector.data(), sizeof(indirect_draw_indexed_command) * DrawCount, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	...
	DirectWindow.Gfx->Device->CreateShaderResourceView(IndirectDrawIndexedCommands.Handle.Get(), &GfxSrvDesc0, DirectWindow.Gfx->GfxResourceHeap->GetCPUDescriptorHandleForHeapStart());
	...
	CmpDescriptorHeapHandle.ptr += 1 * DirectWindow.Gfx->ResourceHeapIncrement;
	...
	// Graphics Resources
	D3D12_ROOT_PARAMETER GfxRootParameter0 = {};
	GfxRootParameter0.ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	GfxRootParameter0.ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;
	GfxRootParameter0.Descriptor = {0, 0};
	CD3DX12_ROOT_SIGNATURE_DESC GfxRootSignatureDesc;
	GfxRootSignatureDesc.Init(1, &GfxRootParameter0, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
	...
	D3D12_INDIRECT_ARGUMENT_DESC DrawArgumentDescs[2] = {};
	...
	DrawArgumentDescs[1].Type = D3D12_INDIRECT_ARGUMENT_TYPE_DRAW;
	...
	D3D12_INDIRECT_ARGUMENT_DESC DrawIndexedArgumentDescs[2] = {};
	...
	DrawIndexedArgumentDescs[1].Type = D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED;
	...
	while(DirectWindow.IsRunning())
	{
		...
		if(!DirectWindow.IsGfxPaused)
		{
			...
			DirectWindow.Gfx->Dispatch(DrawCount / 32, 1, 1);
			DirectWindow.Gfx->EndCompute(IndirectDrawCommands, IndirectDrawIndexedCommands);

			DirectWindow.Gfx->BeginRender(GfxRootSignature0.Get(), IndirectDrawIndexedCommands);
